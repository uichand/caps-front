  ...
  JEONSE_API_URL,
    ...
    if (data.status === "safe") {
      ...
      resultMessage.innerHTML =
        "안전: 깡통전세 가능성이 낮습니다.<br>그래도 등기부등본 확인을 권장합니다.";

      resultDetails.innerText =
        `[${data.method}] 기준가 ${Math.round(
          data.threshold_price
        ).toLocaleString()}만원 이내\n최근 거래 ${data.transaction_count}건`;

    } else if (data.status === "danger") {
      resultBox.className = "danger";

      resultMessage.innerHTML =
        "경고: 깡통전세 가능성이 높습니다.<br>등기부등본 확인이 반드시 필요합니다.";

      resultDetails.innerText =
        `[${data.method}] 기준가 ${Math.round(
          data.threshold_price
        ).toLocaleString()}만원 초과\n최근 거래 ${data.transaction_count}건`;

    } else {
      resultBox.className = "unknown";

      resultMessage.innerText = "판단 불가";

      resultDetails.innerText =
        data.message || "거래 데이터가 부족합니다.";
    }
